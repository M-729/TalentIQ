...
// ApplicationsFilterBar.tsx/AssessmentsFilterBar.tsx/InterviewsFilterBar.tsx/
// EmailActivityFilterBar.tsx's own FilterChip exactly, for a consistent
// toolbar language across every list page.
function FilterChip({
  id,
  label,
  icon: Icon,
...
export function OffersFilterBar({ searchInput, onSearchInputChange, jobId, onJobIdChange, status, onStatusChange, jobs }: OffersFilterBarProps) {
  return (
    <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
      ...
      <FilterChip id="offers-job-filter" label="Filter by job" icon={Briefcase} value={jobId} onChange={onJobIdChange}>
      ...
      <FilterChip
        ...
        icon={ListFilter}
        value={status}
        onChange={(value) => onStatusChange(value as OfferStatus | "")}
      >

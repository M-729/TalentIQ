...
interface UseDeactivateTeamMemberResult {
  run: (userId: string) => Promise<TeamMember | null>;
  isSubmitting: boolean;
  error: string | null;
}

export function useDeactivateTeamMember(): UseDeactivateTeamMemberResult {
  ...
  const run = useCallback(
    async (userId: string): Promise<TeamMember | null> => {
      ...
      setError(null);
      try {
        const { member } = await teamApi.deactivateTeamMember(userId);
        return member;
      } catch (err) {

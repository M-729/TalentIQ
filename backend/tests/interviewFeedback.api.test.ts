...
describe("Interview Feedback API", () => {
  ...
  async function scheduleAndComplete(interviewerIds: string[] = [aliceA.id, bobA.id]) {
    const { application, interviewId, interviewPublicId } = await scheduleOnly(interviewerIds);
    await request(app).patch(`${interviewUrl(interviewPublicId)}/complete`).set("Authorization", authHeaderFor(hrA, companyA.id)).send({});
    return { application, interviewId, interviewPublicId };
  }
  ...
  describe("permissions", () => {
    ...
    it("allows an assigned interviewer to create a draft", async () => {
      const { interviewId, interviewPublicId } = await scheduleAndComplete();
      const res = await request(app)
        .put(feedbackMeUrl(interviewPublicId))
    ...
    it("returns 404 for a cross-company feedback list request", async () => {
      const { interviewId, interviewPublicId } = await scheduleAndComplete();
      const res = await request(app).get(feedbackUrl(interviewPublicId)).set("Authorization", authHeaderFor(hrB, companyB.id));
      expect(res.status).toBe(404);
    });
    ...
    it("rejects an unauthenticated request to the feedback list with 401", async () => {
      const { interviewId, interviewPublicId } = await scheduleAndComplete();
      const res = await request(app).get(feedbackUrl(interviewPublicId));
      expect(res.status).toBe(401);
    });

    it("rejects an unauthenticated draft save with 401", async () => {
      const { interviewId, interviewPublicId } = await scheduleAndComplete();
      const res = await request(app).put(feedbackMeUrl(interviewPublicId)).send({ summary: "x" });
      expect(res.status).toBe(401);
    });
  });
  ...
  describe("state gating", () => {
    ...
    it("blocks draft save while the Interview is still scheduled", async () => {
      const { interviewId, interviewPublicId } = await scheduleOnly();
      const res = await request(app)
        .put(feedbackMeUrl(interviewPublicId))
        .set("Authorization", authHeaderFor(aliceA, companyA.id))
    ...
    it("reports can_edit: false on the roster while the Interview is still scheduled", async () => {
      const { interviewId, interviewPublicId } = await scheduleOnly();
      const res = await request(app).get(feedbackUrl(interviewPublicId)).set("Authorization", authHeaderFor(aliceA, companyA.id));
    ...
    it("blocks submission once the Interview is cancelled", async () => {
      ...
      const res = await request(app)
        .post(feedbackSubmitUrl(interviewPublicId))
        .set("Authorization", authHeaderFor(aliceA, companyA.id))
  ...
  describe("draft", () => {
    ...
    it("never exposes another interviewer's unfinished draft content to a same-company HR", async () => {
      ...
      const res = await request(app).get(feedbackUrl(interviewPublicId)).set("Authorization", authHeaderFor(carolA, companyA.id));
      expect(res.status).toBe(200);
    ...
    it("never exposes another interviewer's draft to a different ASSIGNED interviewer either", async () => {
      ...
      const res = await request(app).get(feedbackUrl(interviewPublicId)).set("Authorization", authHeaderFor(bobA, companyA.id));
      expect(res.body.viewer.feedback).toBeNull();
    ...
    it("shows not_started for an assigned interviewer who has not begun feedback", async () => {
      const { interviewId, interviewPublicId } = await scheduleAndComplete();
      const res = await request(app).get(feedbackUrl(interviewPublicId)).set("Authorization", authHeaderFor(hrA, companyA.id));
  ...
  describe("multiple interviewers", () => {
    ...
    it("reports feedback_progress as 0/2, then 1/2, then 2/2", async () => {
      ...
      let detail = await request(app).get(interviewUrl(interviewPublicId)).set("Authorization", authHeaderFor(hrA, companyA.id));
      ...
      detail = await request(app).get(interviewUrl(interviewPublicId)).set("Authorization", authHeaderFor(hrA, companyA.id));
    ...
    it("reports feedback_progress as null for a still-scheduled Interview", async () => {
      const { interviewId, interviewPublicId } = await scheduleOnly();
      const detail = await request(app).get(interviewUrl(interviewPublicId)).set("Authorization", authHeaderFor(hrA, companyA.id));
  ...
  describe("pipeline behavior", () => {
    ...
    it("does not change Application.status or Application.current_step_id on submission", async () => {
      const { application, interviewId, interviewPublicId } = await scheduleAndComplete();

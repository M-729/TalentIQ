...
describe("Team Members management", () => {
  ...
  describe("POST /team/members/:userId/deactivate", () => {
    ...
    it("39. returns 404 when trying to deactivate a user in another company", async () => {
      const res = await request(app).post(deactivateUrl(hrB.id)).set("Authorization", authHeaderFor(adminA, companyA.id)).send({});
    ...
    it("40. an ADMIN cannot deactivate their own account from this flow", async () => {
      const res = await request(app).post(deactivateUrl(adminA.id)).set("Authorization", authHeaderFor(adminA, companyA.id)).send({});
    ...
    it("HR cannot deactivate anyone", async () => {
      const res = await request(app).post(deactivateUrl(hrA.id)).set("Authorization", authHeaderFor(hrA, companyA.id)).send({});
      expect(res.status).toBe(403);
    });
    ...
    it("refuses to deactivate an ADMIN target (only HR accounts are managed here)", async () => {
      ...
      const res = await request(app).post(deactivateUrl(secondAdmin.id)).set("Authorization", authHeaderFor(adminA, companyA.id)).send({});
      expect(res.status).toBe(409);
    });

    it("rejects deactivating an already-deactivated user with a clear conflict", async () => {
      await request(app).post(deactivateUrl(hrA.id)).set("Authorization", authHeaderFor(adminA, companyA.id)).send({});
      const res = await request(app).post(deactivateUrl(hrA.id)).set("Authorization", authHeaderFor(adminA, companyA.id)).send({});
      expect(res.status).toBe(409);
    });
  ...
  describe("POST /team/members/:userId/reactivate", () => {
    it("38. ADMIN can reactivate a previously-deactivated HR", async () => {
      ...
      const res = await request(app).post(reactivateUrl(hrA.id)).set("Authorization", authHeaderFor(adminA, companyA.id)).send({});
      expect(res.status).toBe(200);
    ...
    it("39. returns 404 when trying to reactivate a user in another company", async () => {
      await request(app).post(deactivateUrl(hrB.id)).set("Authorization", authHeaderFor(adminB, companyB.id)).send({});
      const res = await request(app).post(reactivateUrl(hrB.id)).set("Authorization", authHeaderFor(adminA, companyA.id)).send({});
      expect(res.status).toBe(404);
    });
    ...
    it("HR cannot reactivate anyone", async () => {
      ...
      const res = await request(app).post(reactivateUrl(hrA.id)).set("Authorization", authHeaderFor(anotherActiveHr, companyA.id)).send({});
      expect(res.status).toBe(403);
    });
  });
});
